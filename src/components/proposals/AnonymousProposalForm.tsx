...
export function AnonymousProposalForm({ dancerId, dancerName, onClose }: AnonymousProposalFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (emailResponse.status === 'rejected') {
        ...
        // 이메일 전송 실패는 경고만 표시
        toast.warning('제안은 접수되었으나 이메일 전송에 실패했습니다.')
      ...
      toast.success('익명 제안이 성공적으로 전송되었습니다! 댄서가 확인 후 연락드리겠습니다.')
      onClose()
      router.refresh()
      
    } catch (error) {
      console.error('Anonymous proposal submission error:', error)
    ...
    <Card className="w-full max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle className="flex items-center space-x-2">
          ...
          <span>익명 섭외 제안</span>
          <Badge variant="secondary" className="ml-2">로그인 불필요</Badge>
        </CardTitle>
        <p className="text-sm text-zinc-600">
          <strong>{dancerName}</strong>님에게 익명으로 섭외 제안을 보냅니다.
        </p>
      </CardHeader>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-zinc-900 flex items-center space-x-2">
